...
""".strip()
    ...
    def recognize_image_with_fallback(
        ...
        if not self.vision_available:
            raise AIServiceError("Image recognition service is not configured.")

        primary_error: Optional[Exception] = None
        if settings.has_gemini():
            try:
                return self.recognize_image(image_data, image_type)
            except AIServiceError as error:
                primary_error = error
                logger.warning("Gemini image recognition failed, fallback will be attempted: %s", error)

        if settings.has_backup_text_model():
            try:
                text = self._backup_image_recognition_completion(
                ...
                logger.error("Backup image recognition failed: %s", backup_error)
                if primary_error:
                    logger.error("Primary recognition failure before backup: %s", primary_error)
    ...
    def recognize_image_from_url_with_fallback(self, image_url: str) -> ImageRecognitionResult:
        try:
            response = self._http.get(image_url)
            response.raise_for_status()
        except httpx.HTTPError as error:
        ...
        content_type = response.headers.get("content-type", "image/jpeg").split(";")[0].strip() or "image/jpeg"
        return self.recognize_image_with_fallback(response.content, content_type)
    ...
    def generate_recipe_image(
        ...
        if settings.has_backup_image_model():
            try:
                image_bytes, mime_type, remote_url = self._generate_image_with_backup(prompt)
                return image_bytes, mime_type, prompt, remote_url
            except AIServiceError as backup_error:
                primary_error = backup_error
                logger.warning("Backup image generation failed, Gemini image fallback will be attempted: %s", backup_error)
        ...
        if settings.has_gemini():
            try:
                ...
                return image_bytes, mime_type, prompt, None
            except AIServiceError as error:
                logger.error("Gemini image generation failed after backup image attempt: %s", error)
                if primary_error:
                    logger.error("Primary image failure before Gemini fallback: %s", primary_error)
                raise AIServiceError(self.USER_RETRY_IMAGE_MESSAGE) from error
    ...
    def _generate_image_with_backup(self, prompt: str) -> Tuple[Optional[bytes], str, Optional[str]]:
        ...
        image_timeout = httpx.Timeout(
            connect=max(settings.backup_timeout_connect_seconds, 15.0),
            read=max(settings.backup_timeout_read_seconds, 120.0),
            write=max(settings.backup_timeout_write_seconds, 120.0),
            pool=max(settings.backup_timeout_pool_seconds, 15.0),
        )

        try:
            response_json = self._backup_request("/images/generations", payload, timeout=image_timeout)
        except AIServiceError as error:
        ...
        if image_url:
            return None, "image/jpeg", image_url
    ...
    def _backup_image_recognition_completion(
        self,
        image_data: bytes,
        image_type: str,
        ...
        payload: Dict[str, Any] = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are an ingredient recognition assistant. Return strict JSON only.",
                },
                {
                    "role": "user",
                    "content": [
        ...
        try:
            ...
            if require_json_mode and self._supports_response_format_retry(str(error)):
                logger.warning(
                    "Backup image recognition API rejected response_format, retrying without response_format."
                )
                payload.pop("response_format", None)
                response_json = self._backup_request("/chat/completions", payload)
            else:
    ...
    def _backup_request(
        ...
        *,
        ...
        backup_timeout = timeout or httpx.Timeout(
            connect=settings.backup_timeout_connect_seconds,
            read=settings.backup_timeout_read_seconds,
            write=settings.backup_timeout_write_seconds,
            pool=settings.backup_timeout_pool_seconds,
        )
        max_attempts = max(1, int(retry_attempts or settings.backup_request_retry_attempts or 1))
        last_network_error: Optional[Exception] = None

        for attempt in range(1, max_attempts + 1):
            try:
                response = self._http.post(url, headers=headers, json=payload, timeout=backup_timeout)
                response.raise_for_status()
                ...
                logger.warning(
                    "Backup model timeout on attempt %s/%s: %s",
                    attempt,
                    max_attempts,
                    error,
                )
                ...
                last_network_error = error
                logger.warning(
                    "Backup model network error on attempt %s/%s: %s",
                    attempt,
                    max_attempts,
                    error,
                )
        ...
        logger.error("Backup model network error: %s", last_network_error)
        raise AIServiceError(f"Backup model network error: {last_network_error}") from last_network_error
